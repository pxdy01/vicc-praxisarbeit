...
Flask-REST-API als Docker-Container auf **Azure App Service (Linux)** mit
...
```
Internet ──HTTPS──> App Service (Linux, B1) ── Container patrikzauggipso/vicc-api:2.0.0
...
|---|---|---|---|
| GET | `/` | HTML-Übersicht (Browser) | 200 |
| GET | `/health` | prüft DB-Verbindung | 200 / 503 |
| GET | `/tasks` | alle Tasks | 200 |
| POST | `/tasks` | `{"title": "...", "done": false}` | 201 / 400 |
| GET | `/tasks/{id}` | ein Task | 200 / 404 |
| PUT | `/tasks/{id}` | `{"title": "..."}` und/oder `{"done": true}` | 200 / 400 / 404 |
| DELETE | `/tasks/{id}` | Task löschen | 204 / 404 |

Konfiguration über Umgebungsvariablen: `DB_HOST`, `DB_PORT`, `DB_NAME`,
`DB_USER`, `DB_PASSWORD`, `DB_SSLMODE`, optional `PORT` (Default 8000).
Die Tabelle `tasks` wird beim ersten Zugriff automatisch angelegt.
...
docker build --platform linux/amd64 `
  -t patrikzauggipso/vicc-api:2.0.0 `
...
docker run -d --name vicc-api-local --network vicc-local -p 8000:8000 `
  ...
  patrikzauggipso/vicc-api:2.0.0
...
```powershell
docker push patrikzauggipso/vicc-api:2.0.0
